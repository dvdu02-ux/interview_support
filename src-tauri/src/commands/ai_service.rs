...
pub async fn ai_generate_answer(
    ...
    timeout(Duration::from_secs(60), client.chat_completion_stream(request, channel))
        .await
        .map_err(|_| "AI answer generation timed out after 60s".to_string())?
...
pub async fn ai_detect_question(
    ...
    let request = ChatRequest {
        model: config.model,
        messages: vec![
            AIMessage { role: AIRole::System, content: system_prompt.to_string() },
    ...
    let response = timeout(Duration::from_secs(8), client.chat_completion(request))
        .await
        .map_err(|_| "Question detection timed out after 8s".to_string())?
        .map_err(|e| format!("AI service error: {}", e))?;
